...
static void simpleProfileChangeCB( uint8 paramID )
{
    ...
    case SIMPLEPROFILE_CHAR6: //´®¿ÚÍ¸´« Profile
        SimpleProfile_GetParameter( SIMPLEPROFILE_CHAR6,
                                   newChar6Value );
        ...
        else
        {
            NPI_PrintString("\n");
            NPI_PrintString("\n");
            NPI_PrintValue("newChar6Value [0] = 0x", newChar6Value [0], 16);
            NPI_PrintString("\n");
            NPI_PrintString("\n");
...
static void NpiSerialCallback( uint8 port, uint8 events )
{
  ...
  if (events & HAL_UART_RX_TIMEOUT)   //´®¿ÚÓÐÊý¾Ý
  {
    ...
    if(numBytes)
    {
      ...
      NPI_ReadTransport(&buf[1],buf[0]);    //´Ó´®¿Ú¶Á³öÊý¾Ý
