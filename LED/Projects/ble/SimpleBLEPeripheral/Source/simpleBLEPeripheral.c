...
static void NpiSerialCallback( uint8 port, uint8 events )
{
    ...
    {
        uint8 numBytes = 0;
        numBytes = NPI_RxBufLen(); //¶Á³ö´®¿Ú»º³åÇøÓÐ¶àÉÙ×Ö½Ú
        if(numBytes == 0)
        {
            return;
        }
        else
        {
            ...
            if(buffer)
            {
                //¶ÁÈ¡¶ÁÈ¡´®¿Ú»º³åÇøÊý¾Ý£¬ÊÍ·Å´®¿ÚÊý¾Ý
                NPI_ReadTransport(buffer,numBytes);
                //°ÑÊÕµ½µÄÊý¾Ý·¢ËÍµ½´®¿Ú-ÊµÏÖ»Ø»·
...
void SimpleBLEPeripheral_Init( uint8 task_id )
{
    ...
    NPI_InitTransport(NpiSerialCallback);
    //°´³¤¶ÈÊä³ö 2
    NPI_WriteTransport("opengua 1\r\n", 11);
    //°´×Ö·û´®Êä³ö
    NPI_PrintString("opengua 2\r\n");
    //¿ÉÒÔÊä³öÒ»¸öÖµ£¬ÓÃ 10 ½øÖÆ±íÊ¾
    NPI_PrintValue("ÌðÌðµÄ´óÏã¹Ï 1 = ", 168, 10);
    NPI_PrintString("\r\n");
    //¿ÉÒÔÊä³öÒ»¸öÖµ£¬ÓÃ 16 ½øÖÆ±íÊ¾
    NPI_PrintValue("ÌðÌðµÄ´óÏã¹Ï 2 = 0x", 0x88, 16);
    NPI_PrintString("\r\n");
    //young
    ...
    // is halted
    //ÕâÌõÓï¾ä»áÈÃ¿ÕÏÐµÄ CPU ×Ô¶¯½øÈëµÍÆµÒÔ´Ë½µµÍ¹¦ºÄ£¬×¢ÊÍµô´ú±í²»×Ô¶¯ÇÐ»»ÆµÂÊ¡£
    // ·ÀÖ¹´®¿Ú³öÏÖÂÒÂë¡¢¶ª°ü
    // young
    //HCI_EXT_ClkDivOnHaltCmd( HCI_EXT_ENABLE_CLK_DIVIDE_ON_HALT );
    
    //  ½ûÖ¹ RF ÆÚ¼äÍ£Ö¹ MCU
    // Ä¬ÈÏÊÇ ENABLE µÄ£¬ ENABLE »áÈÃ RF ÆÚ¼äÍ£Ö¹ MCU¡£Òò´ËÐèÒªÌí¼Ó±¾ÌõÓï¾ä£¬¹Ø±ÕËü¡£
